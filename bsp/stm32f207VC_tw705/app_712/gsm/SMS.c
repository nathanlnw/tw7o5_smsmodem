...
void   SMS_protocol (u8 *instr, u16 len, u8  ACKstate)  //  ACKstate
{
    ...
    */
    ...
    if(Vechicle_Info.Vech_Type_Mark == 1)
        strcat(SMS_Service.SMS_sd_Content, "#K1"); // 版本信息
    else if(Vechicle_Info.Vech_Type_Mark == 2)
        strcat(SMS_Service.SMS_sd_Content, "#H1"); // 版本信息
    else
        strcat(SMS_Service.SMS_sd_Content, "#QT"); // QT 版本信息
...
u8 SMS_Rx_Text(char *instr, char *strDestNum)
{
    ...
    rt_kprintf( "\r\n Rx_Text 短息来源号码:%s", SMS_Service.SMS_destNum ); 
...
void sms_dest(u8 *str) 
{
    ...
    else
    {
        ...
        if(strlen((const char *)reg_str) == 11) //  长度判断
        {
            for(i = 0; i < 11; i++)
            {
